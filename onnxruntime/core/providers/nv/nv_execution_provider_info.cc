...
ProviderOptions NvExecutionProviderInfo::ToProviderOptions(const OrtNvProviderOptions& info) {
...
/**
 * Update OrtNvProviderOptions instance with ProviderOptions (map of string-based key-value pairs)
 *
 * Please note that it will reset the OrtNvProviderOptions instance first and then set up the provided provider options
 ...
 *
 * \param provider_options - a pointer to OrtNvProviderOptions instance
 * \param options - a reference to ProviderOptions instance
 * \param string_copy - if it's true, it uses strncpy() to copy 'provider option' string from ProviderOptions instance to where the 'provider option' const char pointer in OrtNvProviderOptions instance points to.
...
void NvExecutionProviderInfo::UpdateProviderOptions(void* provider_options, const ProviderOptions& options, bool string_copy) {
  ...
  auto& nv_provider_options = *reinterpret_cast<OrtNvProviderOptions*>(provider_options);
  nv_provider_options.device_id = internal_options.device_id;

  // The 'has_user_compute_stream' of the OrtNvProviderOptions instance can be set by C API UpdateTensorRTProviderOptionsWithValue() as well
  // We only set the 'has_user_compute_stream' of the OrtNvProviderOptions instance if it is provided in options or user_compute_stream is provided
